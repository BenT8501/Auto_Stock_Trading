...
def filter_trending_stocks(config: dict, market: str | None = None) -> pd.DataFrame:
    """Return trend and volume candidates. This is not a final buy candidate list."""
    ...
    return latest[latest["trend_filter_pass"].astype(bool) & latest["volume_filter_pass"].astype(bool)].reset_index(drop=True)


def build_setup_candidates(config: dict, market: str | None = None) -> pd.DataFrame:
    """Build close-confirmed setup candidates for next-session monitoring."""
    from src.trading.watchlist_builder import build_watchlist

    return build_watchlist(config, market=market)["setup_candidates"]


def build_trigger_candidates(config: dict, market: str | None = None) -> pd.DataFrame:
    """Load latest intraday trigger candidates generated by realtime_scanner."""
    output_dir = config.get("results", {}).get("output_dir", "data/results")
    from pathlib import Path

    files = sorted(Path(output_dir).glob("triggered_candidates_*.csv"))
    if not files:
        return pd.DataFrame()
    frame = pd.read_csv(files[-1], dtype={"symbol": str})
    if market and market.upper() != "ALL":
